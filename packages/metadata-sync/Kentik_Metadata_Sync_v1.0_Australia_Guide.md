...
This package is push-based: a Kentik-side process (configure and enable on Kentik platform) reads device data from the Kentik API and posts it to the ServiceNow Import Set endpoint (`/api/now/import/x_2088674_kentik_0_u_kentik_devices`), authenticated with the REST API key created in section 3. The JSON body sent to ServiceNow should use the same attribute names as Kentik's device metadata — these map 1:1 to the staging table's `import_attribute_name` values. The staging table accepts exactly these 9 attributes:

* `id` — Kentik's device ID; the unique key used to match existing CMDB records
* `deviceName`
* `deviceDescription`
* `createdDate`
* `updatedDate`
* `deleted` — set to `true` to soft-delete/retire the corresponding CI
* `site/id`
* `site/siteName`
* `rawPayload` — the raw JSON response from Kentik's Devices API, which can be used to extract other available fields not covered by the columns above

Each device payload must include Kentik's device `id`, since it is used as the unique key for updating existing CMDB records.

The XML config file is built with ServiceNow Studio and is named `Kentik_Metadata_Sync_v1.0_Australia.xml`. Importing it does the following:

1. Creates a role, `x_2088674_kentik_0.kentik_metadata_syncer`, with create, write, and read permission on the staging table `x_2088674_kentik_0_u_kentik_devices` (write is required so field values submitted on insert are actually persisted — create alone only permits an empty row), plus read permission on `sys_dictionary` (so the role can look up the available columns for a given table) and read permission on `cmdb_ci_netgear`.
...
4. Does **not** set up authentication for you — the REST API Key, its access policy, and the integration user are created manually as part of Section 3 below.
...
3.  Create a new user then tie the user to the API key. When creating the user, give it User ID, First name and Last name. Make Identity type **Machine** and check **Internal Integration User**.
4.  **Submit**. Select the created API key, copy down the generated token — this is the API key value Kentik needs on every request.
5.  Navigate to **System Security** > **Users and Groups** > **Users**, filter for the user created in step 3, and open it.
6.  Edit **Roles** and assign the `x_2088674_kentik_0.kentik_metadata_syncer` role to it — this is what grants the API user access to the staging table and `sys_dictionary`.
7.  **Endpoint to send device data to:**
    ```
    POST https://<instance>.service-now.com/api/now/import/x_2088674_kentik_0_u_kentik_devices
    ```
    Submit one JSON object per device using Kentik's device attribute names as the keys, and the token from step 4 as the API key header.
